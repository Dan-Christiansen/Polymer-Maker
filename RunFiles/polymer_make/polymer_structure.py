...
def polymer_structure(length=1, angle=120):
    ...
    with open('polymer_formula','r') as polyf:
        ...
        for line in doc[1:]:
            ...
            with open(data_path+tmp[-1]+'.gro') as monf:
                mondoc = monf.readlines()
                atms = int(mondoc[1])
            monF[tmp[0]] = (tmp[1],atms)

    anumT = 0 # Total number of atoms in the system to be made
    rotr = 2*np.pi*float(angle)/360 # Input rotation angle between neighboring monomers
